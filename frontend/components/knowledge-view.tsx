...
export function KnowledgeView({ companyId }: { companyId?: string }) {
  ...
  const totals = useMemo(() => sources.reduce((sum, source) => ({ documents: sum.documents + source.documents_count, chunks: sum.chunks + source.chunks_count }), { documents: 0, chunks: 0 }), [sources]);
