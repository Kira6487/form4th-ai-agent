...
export function CompanyDetail({ companyId }: { companyId: string }) {
  ...
  return <section className="content-section narrow-section"><Link className="back-link" href="/dashboard/companies">← Companies</Link><div className="content-header"><div><div className="eyebrow">Company overview</div><h1>{company.name}</h1><p className="content-subtitle">Status: {company.status}</p></div></div><CompanyForm initial={initial} submitLabel="Save changes" onSubmit={async (input) => { const updated = await updateCompany(organization.id, company.id, input); setCompany(updated); }} /><div className="module-links"><Link className="module-card" href={`/dashboard/companies/${company.id}/agent`}><strong>AI Agent</strong><span>Configure and test a grounded business agent.</span></Link><Link className="module-card" href={`/dashboard/companies/${company.id}/knowledge`}><strong>Knowledge</strong><span>Manage website, PDF and manual sources.</span></Link></div></section>;
}
