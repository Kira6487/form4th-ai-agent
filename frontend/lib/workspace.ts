...
import type { AIAgent, ChatMessage, ChatSource, Company, CompanyInput, CompanyUpdate, Conversation, KnowledgeChunk, KnowledgeDocument, KnowledgePage, KnowledgeSearchResult, KnowledgeSource, Lead, Organization } from "../types/workspace";
...
const leadPath = (o: string, c: string) => `/api/v1/organizations/${o}/companies/${c}/leads`;
export const getLeads = (o: string, c: string, params: { status?: string; intent?: string; source?: string; search?: string; page?: number; page_size?: number } = {}) => { const query = new URLSearchParams(Object.entries(params).filter(([, value]) => value !== undefined && value !== "") as [string, string][]).toString(); return apiFetch<{ items: Lead[]; page: number; page_size: number; total: number }>(`${leadPath(o, c)}${query ? `?${query}` : ""}`); };
export const getLead = (o: string, c: string, leadId: string) => apiFetch<Lead>(`${leadPath(o, c)}/${leadId}`);
export const updateLead = (o: string, c: string, leadId: string, input: Partial<Pick<Lead, "status" | "name" | "email" | "phone" | "interest" | "consent_to_contact">>) => apiFetch<Lead>(`${leadPath(o, c)}/${leadId}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(input) });
export async function streamChatMessage(o: string, c: string, conversationId: string, message: string, onText: (text: string) => void, onDone: (sources: ChatSource[], model: string | null, lead: { id: string; status: string } | null) => void): Promise<void> {
